...
this._preco);
  ...
  public static List<Produto> ConsultarProdutos(string encontrarItem)
  {
    List<Produto> produtos = new List<Produto>();
    ...
    using (var connection = new SqliteConnection("Data Source=banco.db"))
    {
      ...
      @"SELECT * 
        FROM produto 
        WHERE produto.nome LIKE '%"+encontrarItem+"%';";
      using (var reader = command.ExecuteReader())
      {
        while (reader.Read())
        {
          ...
          Produto recurso = new Produto(nome, preco);
          recurso._id = id;

          produtos.Add(recurso);
        }
      }
    }
